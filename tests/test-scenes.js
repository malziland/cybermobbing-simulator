QUnit.module('scenes', function () {
  ...
  QUnit.test('All scene i18n keys exist in TRANSLATIONS.de', function (assert) {
    ...
    var hsKeys = ['hs.n1', 'hs.n2', 'hs.n3', 'hs.n4', 'hs.n5', 'hs.n6', 'hs.n7', 'hs.n8'];

    // Keys used in p4b-messages.js
    var imKeys = ['im.mama', 'im.tom'];

    var allKeys = waKeys.concat(igKeys, tkKeys, hsKeys, imKeys);
    allKeys.forEach(function (key) {
      assert.ok(TRANSLATIONS.de[key] !== undefined, 'DE translation exists for scene key: ' + key);
      assert.ok(TRANSLATIONS.en[key] !== undefined, 'EN translation exists for scene key: ' + key);
    });
  });
  ...
  QUnit.test(
    'Character speech pattern comments match the actual characters used (avatars exist)',
    function (assert) {
