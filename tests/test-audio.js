QUnit.module(
  ...
  function () {
    ...
    QUnit.test(
      'Pausing clears all native timeout IDs (remaining time calculated)',
      function (assert) {
        var fixture = document.getElementById('qunit-fixture');
        fixture.innerHTML =
          '<div id="pauseBtn"></div>' +
        ...
        simTimers.forEach(function (timer) {
          assert.ok(
            ...
            'Timer remaining (' + timer.remaining + ') is <= 10000'
          );
    ...
    QUnit.test('initAudio() creates an AudioContext (if available)', function (assert) {
      ...
      if (typeof AudioContext !== 'undefined' || typeof webkitAudioContext !== 'undefined') {
        ...
        initAudio();
        assert.ok(
          ax instanceof (window.AudioContext || window.webkitAudioContext),
