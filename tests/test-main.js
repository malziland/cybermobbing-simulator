QUnit.module(
  ...
  function () {
    QUnit.test('simStarted guard: go() is protected by flag', function (assert) {
      ...
      go();
      assert.ok(
        !document.getElementById('start').classList.contains('gone'),
    ...
    QUnit.test('mkPhoto() sets data-user and data-likes attributes on igfr', function (assert) {
      ...
      assert.equal(
        igfr.getAttribute('data-likes'),
        t('photo.igLikes'),
        'data-likes matches i18n key'
      );
    });
    ...
    QUnit.test('shareSimulation() shows toast with correct i18n text', function (assert) {
      ...
      assert.equal(
        toastEl.textContent,
        t('ui.linkCopied'),
        'Toast text matches ui.linkCopied translation'
      );
    ...
    QUnit.test(
      'mkPhoto() creates elements with i18n text (photo.ugly, photo.exposed, etc.)',
      function (assert) {
        var fixture = document.getElementById('qunit-fixture');
    ...
    QUnit.test('go() resets simTimers to empty array', function (assert) {
      ...
      var found = simTimers.some(function (t) {
        return t.id === 999;
      });
    ...
    QUnit.test(
      'shareSimulation() uses navigator.share on mobile user agents (mock test)',
      function (assert) {
        var fixture = document.getElementById('qunit-fixture');
